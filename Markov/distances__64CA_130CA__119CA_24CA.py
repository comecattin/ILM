...
def get_dir(DATADIR, OUTDIR):
    ...
    GSfiles = sorted(
        [
            f"{DATADIR}/{fn}"
            for fn in allfiles
            if (fn.startswith("GS") and fn.endswith(".xtc"))
        ]
    )
    ESfiles = sorted(
        [
            f"{DATADIR}/{fn}"
            for fn in allfiles
            if (fn.startswith("ES") and fn.endswith(".xtc"))
        ]
    )
    TRJ15files = sorted(
        [
            f"{DATADIR}/{fn}"
            for fn in allfiles
            if (fn.startswith("trj15") and fn.endswith(".xtc"))
        ]
    )
...
def pyemma_feat(pdb, pairNames, refGS, refES):
    ...
    # Get the indices
    pair_indices = create_pairIndices_from_pairNames(
        pdbfilename=refGS, pairNames=pairNames
    )
    # Add the feature
...
def load_allxtc(allxtc, feat):
    ...
    """
    data = pyemma.coordinates.load(allxtc, features=feat, stride=1)
    return data


def plot_density_free_energy(
    data_concatenated, allxtc, timestep, pairNames, OUTDIR, kT
):
    ...
    plt.title(
        f"All {len(allxtc)}  simulations, stored every {timestep} ns", fontweight="bold"
    )
