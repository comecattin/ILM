...
def main():
    ...
    parser = argparse.ArgumentParser(
        "CLI for water simulations",
        description="Compute the molecular volume of water.",
    )
    ...
    no_option = (
        args.output_volume == False
        and args.cutoff == False
        and args.simulation_length == False
    )
    if no_option:
        ...
        if args.output_volume:
            ...
            if args.file_name == None or args.error_file == None:
                raise Exception(
                    "No file or no error file provided. Please use the -h option."
                )
                ...
                # Plot the result
                water_model.plot_volume(
                    xlabel=xlabel,
                    ylabel=ylabel,
                    output_name=args.output_pdf,
                    color=color,
                    show=args.plot,
                )
                # Save the result
        ...
        if args.cutoff:
            ...
            (
                ...
                volume_mean_list,
                volume_error_list,
            ) = water_model.cutoff_extract_volume(args.cutoff_dir)
            water_model.cutoff_plot(
                args.cutoff_dir, color, show=args.plot, output_path=args.cutoff_pdf
            )
            water_model.cutoff_write(args.cutoff_dir, args.cutoff_txt)
        ...
        if args.simulation_length:
            ...
            (
                ...
                volume_mean_list,
                volume_error_list,
            ) = water_model.simulation_length_extract_volume(args.simulation_length_dir)
            water_model.simulation_length_plot(
            ...
            water_model.simulation_length_write(
                args.simulation_length_dir, args.simulation_length_txt
            )
